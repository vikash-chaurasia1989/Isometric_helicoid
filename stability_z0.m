...
f = [b(1:3*N1)  rho' lm' u v w  ]'  ;         % for fun_jacobian2

%  f = [b(1:3*N1)  rho' lm' u v w w1 w2 ]'  ;         % for fun_jacobian4

% %=== for fun_jacobian ===
% bext = load(strbext);
% f = [b(1:3*N1)  rho' lm' u v w  bext']'  ;         % for fun_jacobian6
% %============
...
%st = fun_stability2(f);
%--- testing the numerical jacobian ;

% jac_num = jacobian(@fun_eval_z0,f);
%
% plot(jac-jac_num);
...
dbx(3:N,1)     = x(3:3:3*N1-3,1)    ;
dby(3:N,1)     = x(4:3:3*N1-2,1)    ;
dbz(3:N-1,1)   = x(5:3:3*N1-4,1)    ;
