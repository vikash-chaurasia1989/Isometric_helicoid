...
global  N  N1  u v w    f_b    len id uni err qd bx2 E count bxN h tau rho lm x0 c gamma bt 
global bx by bz bxp byp bzp bx2p by2p bz2p bx4p by4p bz4p bext fac k bx0 by0 bz0 v0 ht x2
...
%    %=====================================================
  ...
  p = 2;
        ...
        f_b(3*p-5,1) = prefac_x*(fac*bx4p(p-1,1) + lmp(p-1)*bxp(p,1) + lm(p)*bx2p(p,1) - rho(p-1)*bx(p,1) + gmx) + (bx(p,1)-bx0(p,1))/bt/ht ;
        f_b(3*p-4,1) = prefac_x*(fac*by4p(p-1,1) + lmp(p-1)*byp(p,1) + lm(p)*by2p(p,1) - rho(p-1)*by(p,1) + gmy) + (by(p,1)-by0(p,1))/bt/ht;
        f_b(3*p-3,1) = prefac_x*(fac*bz4p(p-1,1) + lmp(p-1)*bzp(p,1) + lm(p)*bz2p(p,1) - rho(p-1)*bz(p,1) + gmz) + (bz(p,1)-bz0(p,1))/bt/ht;  
...
%   
  ...
  jac(ind,ind)     = id*(6*fac/h^4 + lmp(p-1,1)/h -2*lm(p)/h^2-rho(p-1) + 1/bt/ht) ;  
  ...
  %jac(5*N1+5,ind) =   [(bx(p,1)-bx0(p,1)) (by(p,1)-by0(p,1)) (bz(p,1)-bz0(p,1))]      ;     
...
% %         
 ...
 % f_b(5*N1+5,1) = 1/2*(sum(sqrt((bx-bx0).^2 + (by-by0).^2 + (bz-bz0).^2)) - d^2);
...
f_b(5*N1+4+indv,1) = (1-V(:,1)'*V(:,1))*Ac*V(:,1)  + (V(:,1)-v0(:,1))/gamma/ht ; 
...
for i = 2:k 
   ...
   temp = 0;
   for p = i-1:-1:1
      % temp = temp + V(:,p)'*V(:,p);                     % Hisod 
      temp = temp +  V(:,p)'*V(:,p)*(Ac + Ac')*V(:,p);    % GHisod 
   end
   
   
  % f_b(5*N1+4+indv,1) = (1-V(:,i)'*V(:,i) - 2*temp)*Ac*V(:,i)  +  (V(:,i)-v0(:,i))/gamma/ht ;   % HiSOD
    f_b(5*N1+4+indv,1) = (1-V(:,i)'*V(:,i))*Ac*V(:,i) - temp   + (V(:,i)-v0(:,i))/gamma/ht ; % GHiSOD 
