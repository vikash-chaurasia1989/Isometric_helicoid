...
for p2 in range(6,1,-1):
    ...
    def initial_guess():
        ...
        if(p2==0):
            str0 = '7pi_knot_N84_tau_245000000000.txt'#'7pi_knot_N84_tau_237000000000.txt'
        else:

            tau2 = int(np.around(tau1[p2+1]*(10**10),decimals=0))
            tau3 = int(np.around(tau1[p2+2]*(10**10),decimals=0))

            str2 = '3fold_N72_tau_' + str(tau2) + '.txt'#'7pi_knot_N84_tau_' + str(tau2) + '.txt'
            str3 = '3fold_N72_tau_' + str(tau3) + '.txt' #'7pi_knot_N84_tau_' + str(tau3) + '.txt'
        ...
        strb2    = path + 'b_' + str2
        strlm2   = path + 'lm_' + str2
        strrho2  = path + 'rho_' + str2
        struvw2  = path + 'uvw_' + str2
        strbext2 = path + 'bext_' + str2

        strb3    = path + 'b_' + str3
        strlm3   = path + 'lm_' + str3
        strrho3  = path + 'rho_' + str3
        struvw3  = path + 'uvw_' + str3
        strbext3 = path + 'bext_' + str3



        points2 = np.loadtxt(strb2)
        points3 = np.loadtxt(strb3)
    ...
    #   Initialization
        bx2 = np.transpose(np.zeros(N+1))
        by2 = np.transpose(np.zeros(N+1))
        bz2 = np.transpose(np.zeros(N+1))

        bxp2 = np.transpose(np.zeros(N+1))
        byp2 = np.transpose(np.zeros(N+1))
        bzp2 = np.transpose(np.zeros(N+1))


        bx2p2 = np.transpose(np.zeros(N+1))
        by2p2 = np.transpose(np.zeros(N+1))
        bz2p2 = np.transpose(np.zeros(N+1))

        bx4p2 = np.transpose(np.zeros(N-1))
        by4p2 = np.transpose(np.zeros(N-1))
        bz4p2 = np.transpose(np.zeros(N-1))

        bext2 = np.transpose(np.zeros(6))
    ...
    #   Initialization
        bx3 = np.transpose(np.zeros(N+1))
        by3 = np.transpose(np.zeros(N+1))
        bz3 = np.transpose(np.zeros(N+1))

        bxp3 = np.transpose(np.zeros(N+1))
        byp3 = np.transpose(np.zeros(N+1))
        bzp3 = np.transpose(np.zeros(N+1))


        bx2p3 = np.transpose(np.zeros(N+1))
        by2p3 = np.transpose(np.zeros(N+1))
        bz2p3 = np.transpose(np.zeros(N+1))

        bx4p3 = np.transpose(np.zeros(N-1))
        by4p3 = np.transpose(np.zeros(N-1))
        bz4p3 = np.transpose(np.zeros(N-1))
        ...
        points = np.loadtxt(struvw2)
        u2 = points[0]
        v2 = points[1]
        w2 = points[2]

        points = np.loadtxt(struvw3)
        u3 = points[0]
        v3 = points[1]
        w3 = points[2]

        #== lagrange multiplier rho for unity constraint
        rho2 = np.loadtxt(strrho2)
        rho3 = np.loadtxt(strrho3)

        #rho = points[:,0]
        #rho = 0*np.ones(N1)
        lm2 = np.loadtxt(strlm2)
        lm3 = np.loadtxt(strlm3)
        #lm  = points[:,0]
        #lm  = 0*np.ones(N1+2)


        bext2 = np.loadtxt(strbext2)
        bext3 = np.loadtxt(strbext3)
        #bext= points[:,0]
    ...
    str0    = str0 = '3fold_N72_tau_' + str(tau2) + '.txt'#'7pi_knot_N' +str(len(bx)-1)+ '_tau_'+ str(tau2) + '.txt'
    strb    = path + 'b_' + str0
    strlm   = path + 'lm_' + str0
