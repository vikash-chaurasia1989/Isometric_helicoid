...
global  N  N1  u v w    f_b    len id uni err qd bx2 E count bxN h tau rho lm bx0 by0 bz0 c ht
global bx by bz bxp byp bzp bx2p by2p bz2p bx4p by4p bz4p bext J2 fac  d
...
%    %=====================================================
  ...
  p = 2;
        ...
        f_b(3*p-5,1) = fac*bx4p(p-1,1) + lmp(p-1)*bxp(p,1) + lm(p)*bx2p(p,1) - rho(p-1)*bx(p,1) + gmx + (bx(p,1)-bx0(p,1))/c/ht ;
        f_b(3*p-4,1) = fac*by4p(p-1,1) + lmp(p-1)*byp(p,1) + lm(p)*by2p(p,1) - rho(p-1)*by(p,1) + gmy + (by(p,1)-by0(p,1))/c/ht;
        f_b(3*p-3,1) = fac*bz4p(p-1,1) + lmp(p-1)*bzp(p,1) + lm(p)*bz2p(p,1) - rho(p-1)*bz(p,1) + gmz + (bz(p,1)-bz0(p,1))/c/ht;  
...
%   
  ...
  jac(ind,ind)     = id*(6*fac/h^4 + lmp(p-1,1)/h -2*lm(p)/h^2-rho(p-1) + 1/c/ht) ;  
