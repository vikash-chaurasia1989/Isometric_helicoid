...
def createplots(N,n):
   ...
   #==== first creating the edges and the midline
    ...
    x  = s
    z1 = a*np.cos(2*PI*n*s)
    y1 = a*np.sin(2*PI*n*s)

    z2 = -a*np.cos(2*PI*n*s)
    ...
    for i in range(0,N+1):
        points1.append([x[i],y1[i],z1[i]])
        points2.append([x[i],y2[i],z2[i]])
        points3.append([x[i],0*y2[i],0*z2[i]])
    ...
    led = 20
    bz1   = np.linspace(z1[0],z2[0],led)
    by1   = np.linspace(y1[0],y2[0],led)
    bx1   = x[0]*np.ones(led)

    bzN   = np.linspace(z1[N],z2[N],led)
