...
global bx,by,bz,bxp,byp,bzp,bx2p,by2p,bz2p,bx4p,by4p,bz4p,bext,f_b,id,tau,jac,ind,lmp,path
...
def initial_guess():
    ...
    global N,N1,u,v,w,f_b,h,tau,rho,lm
    global bx,by,bz,bxp,byp,bzp,bx2p,by2p,bz2p,bx4p,by4p,bz4p,bext,f_b,id,tau,jac,path
    ...
    #str0 = '7pi_knot_N98_tau_160000000000.txt'
    str0 = '7pi_knot_N84_tau_153000000000.0.txt'
    strb    = path + 'b_' + str0
    #strb    = path +  'b_7pi_knot_N84_base.txt';
    strlm   = path + 'lm_' + str0
    ...
    th = PI/2 - .2*np.sin(3*PI*s)
    ph = PI*s

    # initialize array bx by bz

    #bx = np.transpose(np.zeros((N+1)))
    #by = np.transpose(np.zeros((N+1)))
    #bz = np.transpose(np.zeros((N+1)))
    bx = np.multiply(np.sin(th),np.cos(ph))
    by = np.multiply(np.sin(th),np.sin(ph))
    bz = np.cos(th)
    '''
    N  = len(bx)-1
    N1 = N-1
