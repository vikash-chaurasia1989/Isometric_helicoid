...
for p1  = 2:10%length(t1)%100%3:length(steps)%length(tau1)%length(tau1)%length(tau1)%%262:length(tau1)%1:length(tau1)%3:length(tau1)%48:length(tau1)
    
    sv = 1;
    
    d = t1(p1);    % incremental distance between the two curves
    
    var_initial = initial_descent4();
