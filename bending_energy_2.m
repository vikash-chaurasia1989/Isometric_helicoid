...
clc
  ...
  % This is the final version of the code for bending energy of the mobius
  % strip. The dimensionless bending energy is 
...
%sig1 = .1:.2:.9;
%sig1 = 0.5;
sig1 = [0.01 0.015 0.020 ];
sig1 = 0.01;
E  = zeros(length(sig1),length(tau1));
...
for p1 =  1:length(tau1)
    ...
    E(num_sig,p1) = (asinh(n*pi*sig))/(4*pi^3*n^3)*(h*sum(bpp(1:N))/2 - 8*pi^4*n^4) ;
    ...
    plot(tau1(p1)/(2*pi),E(num_sig,p1),'-ok','LineWidth',.5,'color',col{icol})
...
plot(tau1/(2*pi),E(num_sig,:),'--k','LineWidth',.5)
hold on 
