     function  stbl  = fun_stability(x)
 

 %---  This file takes solved data as input and returns 1 if the solution
 %is stable and -1 if unstable
...
%                             bx(p-1,1)*by(p,1) - by(p-1,1)*bx(p,1) ] ; 
  ...
  jac(4*N1+p-1,ind-3)                    = -1/(2*h)*[bxp(p-1,1) byp(p-1,1) bzp(p-1,1)];
...
num_C  = 2*N1+11;   % number of constraints
%
