function x =   initial_descent4()


global   N1  N     id   h   lm    p1 tau c ht N2 k

global bx by bz bxp byp bzp bx2p by2p bz2p bx4p by4p bz4p bx0 by0 bz0 v0 branch
format longE
...
x(1:5*N1+4,1) = x0(1:5*N1+4,1)  - ht*V(I(1));

for i = 1:k 
    
    indv             = (N1-4)*(i-1)+1:(N1-4)*i           ;
    x(5*N1+4+indv,1) = V(:,I(i))./sqrt(sum(V(:,I(i)).^2));   
