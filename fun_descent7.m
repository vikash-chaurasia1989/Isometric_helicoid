...
f_b(1:3*N1,1)  = -bt*prefac_x*Ex(1:3*N1,:)    ;
...
f_b(3*N1+indi,1) = -gamma*(1-V(:,i)'*V(:,i))*(Expv-Exmv)/2/hv/l ;%+ gradGbt ;
...
for i = 2:k
   ...
   % gradGbt =  gradG'*(l*bt/gamma*((gradG*gradG'))\(gradGp-gradGm)/2/hv*(Ex-2*(V(:,i)'*Ex)*V(:,i)));
    ...
    f_b(3*N1+indi,1) = -gamma*(1-V(:,i)'*V(:,i)-2*temp)*(Expv-Exmv)/2/hv/l;%  + gradGbt  ;
