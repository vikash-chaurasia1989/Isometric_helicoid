function [F,jac,m] = fun_dae(t,x,xd)
...
global bx by bz bxp byp bzp bx2p by2p bz2p bx4p by4p bz4p bext J2 fac c
...
% %         
 ...
 %=== multiplying the x_dot and corresponding jacobian entry with the
%relaxation coefficient

% f_b  = xd+c*f_b; 
% jac = c*jac;
% %jac(1:3*N1,:) = c*jac(1:3*N1,:);
% 
% m = eye(5*N1+4);%zeros(1:5*N1+4,1:5*N1+4);
%  
%  F = f_b;
%  
%  

 f_b(1:3*N1,1) = xd(1:3*N1,1)+c*f_b(1:3*N1,1); 
 jac(1:3*N1,:) = c*jac(1:3*N1,:);

m = zeros(5*N1+4,5*N1+4);
m(1:3*N1,1:3*N1) = eye(3*N1); 
