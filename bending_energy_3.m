...
%--This file compares bending energy obtained using \kappa^2 =
%|b''|^2/\tau^2 - \tau^2 and directly computing \kappa from the midline.
%They should be and are same. 
...
%     
    ...
    txg  = [tx(N,1) tx' tx(2,1)];
    tyg  = [ty(N,1) ty' ty(2,1)];
    tzg  = [tz(N,1) tz' tz(2,1)];
  ...
  %  tau2(p1,:) = (rx3p.*(ty'.*rz2p - ry2p.*tz') + ry3p.*(rx2p.*tz' - tx'.*rz2p) + rz3p.*(tx'.*ry2p - rx2p.*ty'))./mod2;
    ...
    tau2(p1,:) = (rx3p.*(ryp.*rz2p - ry2p.*rzp) + ry3p.*(rx2p.*rzp - rxp.*rz2p) + rz3p.*(rxp.*ry2p - rx2p.*ryp))./mod2;
  
    
    txp = (txg(2:N+2)-txg(1:N+1))/h;
    typ = (tyg(2:N+2)-tyg(1:N+1))/h;
    tzp = (tzg(2:N+2)-tzg(1:N+1))/h;
    ...
    E(p1) = h*sum(rx2p.^2+ry2p.^2+rz2p.^2);
    
    E1(p1) = h*sum(bpp(1:N+1))/(tau^2) - tau^2;
